...
function cell_plot = cell_plot(cell_tp,cell_nr)
...
elseif isequal(cell_tp(i).mod,'SUP')  
    ...
    cell_sup_cr = [cell_sup_cr;cell_tp(i).blk(1).tr'];
else
...
% shorten and smooth all traces too 300
    ...
    for j = 1:300
        ...
        b = [b,spk_fac];
        spk_sup = cell_plot.spk_sup(n:(100*j));
        spk_sup = mean(spk_sup);
        c = [c,spk_sup];
        ...
        CR_fac = mean(CR_fac);
        f = [f,CR_fac];
        CR_sup = cell_plot.CR_sup(n:(100*j));
        CR_sup = mean(CR_sup);
        g = [g,CR_sup];
